...
function randomize(xs, rs) { for (var i=0; i<rs.length; i++) xs[i] = xs[i]*(1+(rs[i]*Math.random())); return xs; }
function ballonAnimateOp(c) {
...
to { left: ${p[2]}px; top: ${p[3]}px;  transform: scale(1.2); }`;
	...
	for (var i = 0; i < c.数目; i++) {
		if (c.源点.length == 1) {
            ...
            eLast = addBalloon(randomize(parsePPair(c.源点, c.至点), coeffs), i);
        }
	}
