...
inline fun <T> T.alsoDo(op: Operation): T = also { op.invoke() }

interface DiffAlgorithm {
  fun <E: Any> diff(xs: Feeder<E>, ys: Feeder<E>): Differences<E>
...
interface Sized { val size: Cnt get }
val Sized.isEmpty get() = size == 0
interface FiniteStream { val isEOF: Boolean get }
interface Resetable { fun mark(); fun reset() }
fun <R> Resetable.positional(op: Producer<R>) = mark().let { op().alsoDo { reset() } }
...
interface FinalConsumeIter<out E>: ResetIterator<E>
  { var oneMore: Boolean }
...
open class Feeder<out E: Any>(private val sliter: FinalConsumeIter<E>): PeekStream<E>,
  ...
  override fun consume(): E = peek.alsoDo {
    if (isEOF) throw EOFException()
    Try<IndexOutOfBoundsException, Unit> { lastItem = sliter.next() } 
      ?: run { sliter.oneMore = false }
  }

  private lateinit var lastItem: E
  private lateinit var oldLastItem: E

  protected fun initiate() { lastItem = sliter.next() }
  init { initiate() }
...
object Diff: DiffAlgorithm {
  override fun <E: Any> diff(xs: Feeder<E>, ys: Feeder<E>): DiffAlgorithm.Differences<E> {
    val differences = mutableListOf<Change<E>>()
    fun append(cast: (List<E>) -> Change<E>)
    = fun(list: Collection<E>): Unit
    = list.takeIf { !it.isEmpty() }?.let { it.toList() }?.let(cast)
    ?.let(differences::add)
    ?.let { check(it) }  ?: Unit
    ...
    while (!xs.isEOF && !ys.isEOF) {
      if (xs.peek == ys.peek) {
        buffer.add(xs.consume().alsoDo(ys::consume)); continue }
      same(buffer.toList()); buffer.clear()
      val rest = xs.positional { xs.toSet() }

      ys.mark() //for EOF-rescue
      val ins = Try<EOFException, List<E>> { ys.takeWhile { it !in rest } } ?: run {
        deleted(rest); ys.reset(); ys.toList().also(inserted)
        xs.take(rest.size)
      }
      if (ys.isEOF) break // no `common' succeeder found
    ...
    same(buffer) // with tail-sequence of Same(...)
