...
its (stm.dropWhile);
allok(it(x=>x<4, stmbase.intoIter([1,2,3])).length == 3);
allok(it(x=>x<10, [1,2,3,11,9]).length == 3);
its (stm.zipWith);
...
its (parserc.chars);
allok(stmaux.collect(it("abc"))[2] == 'c', it('')().done);
its (parserc.makeNew);
allok(it(Array)([1,2,3])[2] == 3, it(String)([1]) == '1');

testing('Item feeder stream');
a = parserc.Feeder.prototype;
its (parserc.Feeder);
allok(new it('q').fileName == '<feed>');
s = new it('abc', 'foo.txt');
allok(s.lastItem == undefined, !s.eof(), s.fileName == 'foo.txt');
allok(s.lineNum == 1, s.count == 0, s.col == 0);
its (a.lastIsLF);
allok(!s.lastIsLF());
s.next();
allok(!s.lastIsLF());
s = new parserc.Feeder('\n\r');
allok(s.next() == '\n', s.lastIsLF(), s.next() == '\r', !s.lastIsLF());
its (a.lastIsCR);
s = new parserc.Feeder('\r\n');
allok(s.next() == '\r', s.lastIsCR(), s.next() == '\n', !s.lastIsCR());
its (a.lastIsCRLF);
s = new parserc.Feeder('\r\n');
allok(s.next(), s.lastIsCRLF(), s.next(), !s.lastIsCRLF());
s = new parserc.Feeder('\r');
allok(s.next(), !s.lastIsCRLF(), !s.next(), !s.lastIsCRLF());
s = new parserc.Feeder('\n\rq');
allok(s.next(), !s.lastIsCRLF(), s.next(), !s.lastIsCRLF());
its (a.duplast);
s = new parserc.Feeder('abc');
allok(s.next() == 'a', s.lastItem == 'a', s.nextItem == 'b');
s.duplast();
allok(s.next() == 'a', s.lastItem == 'a', s.nextItem == 'b');
allok(s.next() == 'b', s.lastItem == 'b', s.nextItem == 'c');
allok(s.next(), s.nextItem == undefined);
its (a.next);
s = new parserc.Feeder('abc\r\ndef');
while (s.next()) s.next(); // Why CRLF is so special? should be fixed
allok(s.eof(), s.lastItem == undefined, s.count == 8, s.lineNum == 2, s.col == 3);
its (a.next);
s = new parserc.Feeder('abc\n\rdef. 啊啊啊');
while (s.next()) s.next();
allok(s.eof(), s.nextItem == undefined, s.count == 12+1, s.lineNum == 3, s.col == 8); // 迫真避邪
its (a.next);
s = new parserc.Feeder('abc\n\ndef emmm');
while (s.next()) s.next();
allok(s.eof(), s.count == 13, s.lineNum == 3, s.col == 8);
its (a.next);
s = new parserc.Feeder('abc\r\rdef emmm');
while (s.next()) s.next();
allok(s.count == 13, s.lineNum == 3, s.col == 8);
its (a.next);
s = new parserc.Feeder('abc\rdef\n Hhhhh');
while (s.next()) s.next();
allok(s.count == 14, s.lineNum == 3, s.col == 6); // well behaved, no CRLFCRLF line ending
its (a.eof);
s = new parserc.Feeder('abc');
allok(!s.eof(), s.next(), s.next(), s.next(), is.undef(s.next()), s.eof());
its (a.hasNext);
s = new parserc.Feeder('abc');
allok(s.hasNext(), s.next(), s.next(), s.next(), !s.hasNext());
its (a.loc);
s = new parserc.Feeder('10 + 1');
allok(s.loc() == '<feed>:1:0');
s = new parserc.Feeder('a\nbcd', 'k.txt');
s.next(); s.next(); s.next(); s.next();
allok(s.lastItem == 'c', s.loc() == 'k.txt:2:2');
its (a.desc);
allok(s.desc() == "k.txt:2:2 (pos=4@`c', next=`d')");
s.next();
allok(s.desc() == "k.txt:2:3 (pos=5@`d')");
its (a.logs);
allok(is.null(s.stack), s.logs(1), is.null(s.stack), (s.stack = []), s.logs(1), is.some(s.stack));
its (a.leave);
allok(s.leave(), is.empty(s.stack));
fintest();

testing('Basic parser combinator protocol');
its (parserc.pmatch);
var m = it(1);
its (parserc.pfail);
var f = it(2);
...
var p = null, pp = null;
its (parserc.seq);
p = it([parserc.kwP('abc'), parserc.wsP(), parserc.kwP('def'),
  parserc.ws0P(), parserc.kwP('s')],
  parserc.makeNew(Array));
pp = parserc.run(p).andThen(docall('isRight'));
allok(pp('abc defs'), !pp('abc'), !pp(''), !pp('abcdef s'), !pp('abcdefs'), pp('abc  def s'));
its (parserc.possible);
